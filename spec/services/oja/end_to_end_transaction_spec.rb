...
RSpec.describe "OJA end-to-end transaction lifecycle" do
  it "carries a receiver's multi-plan funding through authorization, order, fulfillment and settlement reconciliation" do
    ...
    order_allocation_a = instance_double(Oja::OrderAllocation, order_id: 501, reservation: reservation_a, amount: 60_000.to_d, currency: "NGN", vendor_reference: "vendor-a", settlement_profile: profile_a)
    order_allocation_b = instance_double(Oja::OrderAllocation, order_id: 501, reservation: reservation_b, amount: 40_000.to_d, currency: "NGN", vendor_reference: "vendor-b", settlement_profile: profile_b)
    ...
    allow(order_allocation_a).to receive(:order_id).and_return(501)
    allow(order_allocation_b).to receive(:order_id).and_return(501)
