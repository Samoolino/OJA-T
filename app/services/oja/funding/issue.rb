    ...
    class Issue
      ...
      def call
        ...
        ActiveRecord::Base.transaction do
          ...
          ingress = Oja::FundingIngress.create!(source_type: @source_type, source_reference: @source_reference, provider: @provider, status: "verified", currency: @currency, amount_minor: @amount_minor, idempotency_key: @idempotency_key.to_s, correlation_id: @correlation_id.to_s, evidence:)
          entry = Oja::FinancialLedgerEntry.create!(allocation_id: allocation.id, operation_id: Oja::Idempotency.operation_id(prefix: "fund", idempotency_key: @idempotency_key), idempotency_key: @idempotency_key.to_s, entry_type: "fund", amount_minor: @amount_minor, currency: @currency, correlation_id: @correlation_id.to_s, source_type: @source_type, source_id: ingress.id.to_s, metadata: @metadata.merge("funding_evidence" => evidence))
          allocation.funded_minor += @amount_minor
          allocation.save!
          Result.new(funded: true, reason: "funding_accepted", amount_minor: @amount_minor, currency: @currency, evidence: evidence.merge("ingress_id" => ingress.id, "operation_id" => entry.operation_id))
        end
